...
public class battleTeamsCommand {
    ...
    private static int attacker(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendFeedback(() ->
                Text.literal("[BattleTeams] You are now an Attacker").formatted(Formatting.RED), false);
    ...
    private static int defender(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendFeedback(() ->
                Text.literal("[BattleTeams] You are now a Defender").formatted(Formatting.BLUE), false);
    ...
    private static int press(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendFeedback(() ->
                Text.literal("[BattleTeams] You must be accepted by staff!").formatted(Formatting.YELLOW), false);

        broadcastToPermission(
                source.getServer(),
                "battleteams.manage",
                Text.literal("[BattleTeams] " + playerName + " is requesting to become press! /battleteams press accept " + playerName).formatted(Formatting.YELLOW)
        );
    ...
    private static int pressAccept(CommandContext<ServerCommandSource> ctx) throws CommandSyntaxException {
        ...
        target.sendMessage(Text.literal("[BattleTeams] You have been accepted as press!").formatted(Formatting.GREEN));

        source.sendFeedback(() ->
                Text.literal("[BattleTeams] Accepted " + playerName + " as press!").formatted(Formatting.GREEN), false);
    ...
    private static int clear(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendFeedback(() ->
                Text.literal("[BattleTeams] All teams cleared!").formatted(Formatting.GREEN), false);
